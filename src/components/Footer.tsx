        ...
        <footer className="bg-slate-50 border-t border-slate-200 pt-16 pb-8 px-6">
            <div className="max-w-7xl mx-auto">
                    ...
                    <div className="space-y-6">
                        <Link to="/" className="flex items-center gap-2 group cursor-pointer">
                            <div className="w-8 h-8 bg-[#4B2E83] rounded flex items-center justify-center">
                                <span className="text-[#C9A227] font-bold text-sm leading-none select-none">{config?.platform_name ? config.platform_name.substring(0,2).toUpperCase() : 'NS'}</span>
                            </div>
                            <span className="text-lg font-extrabold tracking-tight text-[#4B2E83] uppercase">
                                {config?.platform_name ? config.platform_name.split(' ')[0] : 'NUMERO'} <span className="text-[#C9A227]">{config?.platform_name ? config.platform_name.substring(config.platform_name.indexOf(' ') + 1) : 'SANSAR'}</span>
                            </span>
                        </Link>
                    ...
                    <div className="space-y-6">
                        ...
                        <ul className="space-y-4">
                            <li className="flex items-center gap-3 text-sm text-slate-600">
                                ...
                                <span>support@{config?.platform_name ? config.platform_name.toLowerCase().replace(/\s+/g, '') : 'numerosansar'}.com</span>
                            </li>
                            <li className="flex items-center gap-3 text-sm text-slate-600">
                ...
                <div className="pt-8 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
                    <p className="text-xs text-slate-500">{config?.copyright_text || '© 2026 NUMERO SANSAR. All rights reserved.'}</p>
