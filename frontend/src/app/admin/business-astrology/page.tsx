...
export default function BusinessAstrology() {
    // --- State ---
    ...
    const fetchHistory = async (id: string) => {
        try {
            ...
            if (res.ok) {
                ...
                // If editing specific ID, find and populate its data
                if (hasUrlId) {
                    const record = businessHistory.find((h: any) => h.id === Number(hasUrlId));
                    if (record) {
                        setBusinessName(record.name_value);
    ...
    // Lookup Maps
    const { chMap, pyMap, nuMap, harmonicBridges } = useMemo(() => {
        ...
        lettersMap.forEach(l => {
            ...
            const nuNum = Number(l.numerology_number || 0);
            c[l.letter] = chNum;
            p[l.letter] = pyNum;
            n[l.letter] = nuNum;
        ...
        return { chMap: c, pyMap: p, nuMap: n, harmonicBridges: sortedBridges };
    }, [lettersMap]);
    ...
    const [creditType, setCreditType] = useState<"regular" | "whitelabel">("regular");
    ...
    const saveToBackend = async () => {
        ...
        setIsSaving(true);
        try {
            const res = await fetch(`${BASE_URL}/admin/business-astrology/check`, {
                method: "POST",
                headers: {
            ...
            if (clientId && newCheckId) {
                await fetch(`${BASE_URL}/admin/astrology/confirm`, {
                    ...
                    body: JSON.stringify({ check_id: newCheckId, type: 'Business', credit_type: creditType })
                });
                setConfirmed(true);
            }
    ...
    const confirmBusiness = async () => {
        if (!checkId) return;
        try {
            const res = await fetch(`${BASE_URL}/admin/astrology/confirm`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem('admin_token')}`
                },
                body: JSON.stringify({ check_id: checkId, type: 'Business', credit_type: creditType })
            });
            if (res.ok) {
                setConfirmed(true);
                const clientId = searchParams.get('client_id');
    ...
    const downloadPDF = async () => {
        ...
        doc.text("Business Astrology Report", pageWidth / 2, 20, { align: "center" });
        ...
        let tableBody: any[] = [];
        if (showChald) {
            const row: (string | number)[] = [...breakdown.map(b => b.chaldean)];
            if (activeSystemsCount > 1) row.unshift("Chaldean");
            tableBody.push(row);
        }
        if (showPyth) {
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] text-muted-foreground font-bold uppercase tracking-widest mt-1 ml-1 opacity-60">Saved Business Astrology Records</p>
                        </div>
                    </div>
                    <button
                    ...
                    {history.map((record: any) => (
                        <button
                            ...
                            onClick={() => {
                                ...
                                setConfirmed(record.is_confirmed == 1);
                                setShowListing(false);
                            }}
                    ...
                    <div>
                        <div className="flex items-center gap-4">
                            <h1 className="text-3xl font-black flex items-center gap-3 text-foreground tracking-tight">
                                ...
                                {checkId ? 'Edit Analysis' : 'Business Astrology'}
                            </h1>
                ...
                <div className="flex gap-4 items-center">
                    <div className="flex items-center gap-4 ml-4">
                        <div className="flex items-center gap-2">
                            <label className="text-[10px] font-black uppercase text-muted-foreground">Chaldean</label>
                            <button
                                onClick={() => setShowChaldean(!showChaldean)}
                                className={`w-12 h-6 rounded-full transition-all relative border ${showChaldean ? 'bg-[#D4AF37] border-[#D4AF37]' : 'bg-gray-200 border-gray-200'}`}
                            >
                                <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${showChaldean ? 'left-7' : 'left-1'}`} />
                            </button>
                        </div>
                        <div className="flex items-center gap-2">
                            <label className="text-[10px] font-black uppercase text-muted-foreground">Pythagorean</label>
                            <button
                                onClick={() => setShowPythagorean(!showPythagorean)}
                    ...
                    {(chaldeanRes && pythagoreanRes && numerologyRes) && (
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                            ...
                            <div className="bg-white border border-black/5 shadow-xl rounded-3xl p-6 overflow-hidden">
                                ...
                                <div className="overflow-x-auto custom-scrollbar pb-2">
                                    <div className="min-w-max">
                                        ...
                                        {/* Rows */}
                                        {showChaldean && (
                                            <div className="flex items-center py-4 border-b border-border/30 group hover:bg-mystic-gold/10 transition-colors rounded-xl font-medium">
                                                <div className="w-32 text-[10px] font-black text-mystic-gold uppercase px-4 tracking-widest flex items-center gap-2">{(showChaldean && showPythagorean) ? 'Chaldean' : 'Vibrations'}</div>
                                                <div className="flex gap-2">
                                                    {breakdown.map((b, i) => (
                                        ...
                                        {showPythagorean && (
                                            <div className="flex items-center py-4 group hover:bg-indigo-500/10 transition-colors rounded-xl mt-2 font-medium">
                                                <div className="w-32 text-[10px] font-black text-indigo-500 uppercase px-4 tracking-widest flex items-center gap-2">Pythagorean</div>
                                                <div className="flex gap-2">
                                                    {breakdown.map((b, i) => (
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                {showChaldean && chaldeanRes?.description && (
                                ...
                                {showPythagorean && pythagoreanRes?.description && (
                                    <div className="premium-card p-6 rounded-2xl border border-border relative overflow-hidden bg-card/40 backdrop-blur-sm group">
                                        <div className="absolute top-0 left-0 w-1.5 h-full bg-[#6366f1]" />
                                        <h3 className="text-[#6366f1] font-black mb-3 flex items-center gap-3 text-[10px] uppercase tracking-[0.2em]">
                ...
                <div className="xl:col-span-1 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-black/5 sticky top-20 shadow-xl no-print">
                        ...
                        <div className="space-y-10">
                            {selectedSectorId && sectors.find(s => s.id === selectedSectorId) && (
                                ...
                                >
                                    ...
                                    <div className="flex flex-wrap gap-2 relative z-10">
                                        {(sectors.find(s => s.id === selectedSectorId)?.lucky_numbers || '').split(',').filter((n: string) => n.trim() !== '').map((num: string, i: number) => (
                                            <div
                                                key={i}
                            ...
                            {history.length > 0 && !showListing && (
                                <motion.div
                                    initial={{ opacity: 0, y: 20 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="pt-8 border-t border-border/50"
                                >
                                    <h3 className="text-[10px] font-black text-foreground mb-4 uppercase tracking-[0.2em] flex items-center gap-2">
                                    ...
                                    <div className="space-y-3">
                                        {history.map((record: any) => (
                                            <button
                                                ...
                                                className={`w-full text-left p-3 rounded-xl border transition-all flex items-center justify-between group ${checkId === record.id ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-black/5 bg-white hover:border-primary/30'}`}
                                            >
                                                <div className="flex flex-col gap-0.5 min-w-0">
                                                    <span className={`text-sm font-black truncate ${checkId === record.id ? 'text-primary' : 'text-foreground'}`}>
                                                        {record.name_value}
                                                    </span>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[8px] text-muted-foreground font-bold uppercase">{new Date(record.created_at).toLocaleDateString()}</span>
                                                        {record.is_confirmed == 1 && <span className="text-[8px] text-green-500 font-black tracking-widest uppercase">Confirmed</span>}
                                                    </div>
                                                </div>
                                                <ChevronRight size={14} className={`transition-all ${checkId === record.id ? 'text-primary translate-x-0' : 'text-muted-foreground opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0'}`} />
                                            </button>
                                        ))}
                                    </div>
                                </motion.div>
