...
export default function PaymentsPage() {
    ...
    const stats = [
        { label: "Gross Volume", val: `₹${(Array.isArray(transactions) ? transactions : []).reduce((acc, tx) => acc + (tx.status === 'paid' ? parseFloat(tx.amount) : 0), 0).toLocaleString('en-IN')}`, icon: <DollarSign size={20} />, color: "text-[#C9A227]", bg: "bg-[#C9A227]/10" },
        { label: "Platform Net", val: `₹${(Array.isArray(transactions) ? transactions : []).reduce((acc, tx) => acc + (tx.status === 'paid' ? parseFloat(tx.amount) * 0.97 : 0), 0).toLocaleString('en-IN')}`, icon: <CreditCard size={20} />, color: "text-[#4B2E83]", bg: "bg-[#4B2E83]/10" },
        { label: "Failed Ledger", val: (Array.isArray(transactions) ? transactions : []).filter(tx => tx.status === 'failed').length, icon: <ArrowDownRight size={20} />, color: "text-red-500", bg: "bg-red-500/10" },
    ];
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between px-2">
                ...
                <div className="flex gap-3">
                    <button className="px-6 py-3 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-[#4B2E83] hover:border-[#4B2E83]/30 hover:shadow-xl hover:shadow-purple-900/5 transition-all shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {stats.map((stat, i) => (
                    <div key={i} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group hover:shadow-lg transition-all">
                        ...
                        <div>
                            ...
                            <h2 className="text-4xl font-black tracking-tighter text-slate-900 leading-none">{stat.val}</h2>
                        </div>
                    </div>
                ))}
            </div>
            ...
            {/* Search Bar */}
            <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-xl shadow-slate-200/50 flex items-center justify-between">
                <div className="flex items-center gap-4 flex-1 max-w-xl group">
                    <div className="p-3 bg-slate-50 text-slate-300 group-focus-within:text-[#4B2E83] group-focus-within:bg-[#4B2E83]/5 rounded-xl transition-all">
                        <Search size={18} />
                    </div>
                    <input
                        type="text"
                        placeholder="Scan ledger by Invoice ID, Numerologist, or Status..."
                ...
                <div className="flex gap-6 text-[10px] font-black uppercase tracking-widest text-slate-300">
                    <span className="text-[#4B2E83] cursor-pointer">Live Stream</span>
                    <span className="hover:text-[#4B2E83] cursor-pointer">Settled Only</span>
                    <span className="hover:text-[#4B2E83] cursor-pointer">Reversals</span>
                </div>
            </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                ...
                                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Numerologist Origin</th>
                            ...
                            {loading ? (
                                ...
                                <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors group">
                                    <td className="px-10 py-6">
                                        <div className="text-[10px] font-black font-mono text-slate-400 uppercase tracking-widest">#{tx.invoice_id}</div>
                                    </td>
                                    <td className="px-6 py-6">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-400 group-hover:bg-[#4B2E83] group-hover:text-white transition-all shadow-sm">
                                                {(tx.username || 'A')[0].toUpperCase()}
                                            </div>
                                            <div>
                                                <div className="text-sm font-black tracking-tight">{tx.full_name || tx.username}</div>
                                                <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{tx.plan_name}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 font-black text-sm tracking-tight text-[#C9A227]">₹{parseFloat(tx.amount).toLocaleString('en-IN')}</td>
                                    <td className="px-6 py-6">
                                        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm ${tx.status === 'paid' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                                            <div className={`w-1.5 h-1.5 rounded-full ${tx.status === 'paid' ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`}></div>
                                            {tx.status}
                                        </div>
                                    </td>
            ...
            {/* Pagination Controls */}
            <div className="px-4 py-2 flex items-center justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest">
